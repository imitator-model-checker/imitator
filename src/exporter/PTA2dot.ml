...
let color_of_action = fun index ->
	(* If more colors than our array: black *)
	try dot_colors.(index) with Invalid_argument _ -> "black"
...
let string_of_automata (graphics_detail : AbstractAlgorithm.graphics_detail) (options : Options.imitator_options) model =
...
Generation time: " ^ (now()) ^ "\"];" 
	...
	^	(if graphics_detail = Minimal then "\n bgcolor = \"transparent\";" else "")
	^ "\n"
	(* General information *)
	^ (if graphics_detail = Full then title else "")
	^ (if graphics_detail = Full then info_boxes else "")
	^ (
		string_of_list_of_string_with_sep "\n\n" (
...
(* Convert an automaton into a string *)
let string_of_model options model =
	let graphics_detail : AbstractAlgorithm.graphics_detail = options#graphics_detail in 
	(match graphics_detail with
	| Full -> string_of_header options 
	| Minimal -> "")
	^  "\n" ^ string_of_automata graphics_detail options model
