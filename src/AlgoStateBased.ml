...
(*		(* List of states computed before *)
	...
	method private explore_stack_dfs init_state_index =
		...
		*)
		...
		let checkZoneNotEqualInList state_index1 state_index_list= 
			(* Print some information *)
			print_message Verbose_total ("Entering checkZoneNotIncludeInList(" ^ (string_of_int state_index1) ^ ")…");
			...
			(
				try(
					List.iter (fun state_index2 -> 
						...
						print_message Verbose_total ("Comparing with state " ^ (string_of_int state_index2) ^ "…");
						let loc2, constr2 = StateSpace.get_state state_space state_index2 in
						if (checkZoneProjectedOnP state_index1 state_index2) && (Location.location_equal loc1 loc2)
						then  raise (FoundLargerZone);
					) state_index_list;
					true;
				) with FoundLargerZone -> false;
		...
		let select_from_queue () = match options#exploration_order with 
			...
			| Exploration_NestedDFS_with_Subsumption_Synthesis2 -> List.hd !queue;
		...
		while limit_reached = Keep_going && !queue <> [] && !algorithm_keep_going do
			...
			let successors = (match options#exploration_order with
				...
				| Exploration_NestedDFS_with_Subsumption_Synthesis2 -> if !isPrune  then (
																				isPrune := false;
																				[]
																			) else (
																				try(
...
(*		(*Initialization of slast : used in union mode only*)
		...
		match options#exploration_order with
			...
			| Exploration_NestedDFS_with_Subsumption_Synthesis2 -> self#explore_stack_dfs init_state_index;
		end;
