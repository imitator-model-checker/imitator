...
let string_of_guard variable_names x_coord_str y_coord_str = function
	...
	| Discrete_guard discrete_guard ->
		...
		"<label kind=\"guard\" x=\"" ^ x_coord_str ^ "\" y=\"" ^ y_coord_str ^ "\">" ^ (LinearConstraint.string_of_d_linear_constraint variable_names discrete_guard) ^ "</guard>"
	
	| Continuous_guard continuous_guard ->
		(* Remove true guard *)
		
		if LinearConstraint.pxd_is_true continuous_guard then "" else
		"<label kind=\"guard\" x=\"" ^ x_coord_str ^ "\" y=\"" ^ y_coord_str ^ "\">" ^ (LinearConstraint.string_of_pxd_linear_constraint variable_names continuous_guard) ^ "</guard>"
	...
	| Discrete_continuous_guard discrete_continuous_guard ->
		"<label kind=\"guard\" x=\"" ^ x_coord_str ^ "\" y=\"" ^ y_coord_str ^ "\">" ^ (
			...
			(
				...
				else "&&" ^ (LinearConstraint.string_of_pxd_linear_constraint variable_names discrete_continuous_guard.continuous_guard)
			)
		) ^ "</guard>"
