...
let check_normal_update variable_infos automaton_name normal_update =
    ...
    let updated_variable_name_opt = (ParsingStructureUtilities.fold_parsed_normal_update_with_local_variables VariableMap.empty (@) [] (fun _ leaf -> match leaf with Leaf_update_variable variable_name -> [variable_name]) (fun _ _ -> []) normal_update |> List.filter (fun x -> x <> "") |> List.nth_opt) 0 in
