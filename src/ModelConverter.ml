...
let check_normal_update variable_infos automaton_name normal_update =
    ...
    let all_variables_declared = ParsingStructureUtilities.all_variables_defined_in_parsed_normal_update variable_infos print_variable_in_update_not_declared_opt normal_update in
    (* Get (maybe) an updated variable in normal update *)
    let updated_variable_name_opt = (ParsingStructureUtilities.fold_parsed_normal_update (@) [] (function Leaf_update_updated_variable variable_name -> [variable_name] | _ -> []) normal_update |> List.filter (fun x -> x <> "") |> List.nth_opt) 0 in
