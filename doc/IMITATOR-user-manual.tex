...
\begin{itemize}
	...
	\item \label{item:lbl-stack_is_empty} \styleIMI{stack\_is\_empty(s:'a stack):bool} -- Returns \styleIMI{True} iff \styleIMI{s} is empty
...
% NOTE: TODO: all seq-then should be removed from examples and explanations!
...
\begin{itemize}
	\item \styleIMI{queue():'a stack} -- return an empty queue
	\item \label{item:lbl-queue_push} \styleIMI{queue\_push(e:'a, q:'a queue)} -- Add an element \styleIMI{e} at the beginning of the queue \styleIMI{q}---the queue is modified, this function is subject to side-effects
	\item \label{item:lbl-queue_pop} \styleIMI{queue\_pop(q:'a queue):'a} -- Get and remove the last element of the queue \styleIMI{q}---the queue is modified, this function is subject to side-effects
	\item \label{item:lbl-queue_top} \styleIMI{queue\_top(q:'a queue):'a} -- Get the last element of the queue \styleIMI{q}---the queue is left unmodified
	\item \label{item:lbl-queue_clear} \styleIMI{queue\_clear(q:'a queue)} -- Remove all elements of the queue \styleIMI{q}---the queue is modified, this function is subject to side-effects
	\item \label{item:lbl-queue_is_empty} \styleIMI{queue\_is\_empty(q:'a queue):bool} -- Check if the queue \styleIMI{q} is empty
	\item \label{item:lbl-queue_length} \styleIMI{queue\_length(q:'a queue):int} -- Get the number of elements in the queue \styleIMI{q}
\end{itemize}
...
A user-defined function is made of a (possibly empty) list of declarations of local variables (introduced by \styleIMI{let}), of a set of imperative instructions (modification of global variables) and may have one \styleIMI{return} expression.

Local variable declarations and imperative instructions can be written in any order.
A function may or may not return a value.
If a function returns something, the last instruction should be an expression that is the \styleIMI{return} expression.
...
User-defined functions can have zero or more arguments, and must return one value.
Note that all arguments are passed by value except arrays, stacks and queues which are passed by reference.
% TODO: explain more carefully with an example
...
\item Array are passed by reference, as soon as the content of an array passed by reference is modified, the function is labeled internally as a ``side-effect function'';
...
\begin{IMITATORmodel}
	...
	function side_effect_function_2(a : int array(2)) : int
	begin
...
However, side-effect user-defined functions can only be used in updates.
...
Some structure as \styleIMI{function}, \styleIMI{for}, \styleIMI{while}, \styleIMI{if} works as blocs.
...
For example, a variable \styleIMI{x} declared at the top most of the model, a global variable, is accessible at any level of the model.
We can access \styleIMI{x} in any user functions, loops, conditions.
