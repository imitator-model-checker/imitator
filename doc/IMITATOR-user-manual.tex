...
\paragraph{New initial definitions section}

Since \imitator{} 3.1, an \imitator{} model accept a new form of initial definition section that separate discrete initializations from initial constraints. You can see it, commented, on the model above.
...
Note the difference between constraints initializations and discrete variables initializations. The first are initialized in the \styleIMI{continuous} section while other are initialized in the \styleIMI{discrete} section. Moreover, discrete variables are initialized with the assignment operator \styleIMI{:=} in contrast with unification operator \styleIMI{=} used for constraints. As all discrete variables are independent, the initializations are separated with a comma.

As discrete variables are syntactic sugar for locations, you can see that locations are initialized in the same manner as discrete variables in the \styleIMI{discrete} section.

Except this particular elements, this new init state follow the same rules and behavior than the previous init state.
...
A set of rational variables is $\RVar = \{ \rvar_1, \dots, \rvar_\RVarCard \} $;
a rational variable valuation is a function $\rval \colon \RVar \rightarrow \grandn$.

\emph{Integer variables} are bounded signed integer-valued variables encoded on 32 bits.
A set of integer variables is $\IVar = \{ \ivar_1, \dots, \ivar_\IVarCard \} $;
a integer variable valuation is a function $\ival \colon \IVar \in \left[-2147483648, 2147483647\right]$.
Warning : All arithmetic operations over int64 are taken modulo $2^{32}$.

\emph{Boolean variables} are boolean-valued variables.
A set of boolean variables is $\BVar = \{ \bvar_1, \dots, \bvar_\BVarCard \} $;
a boolean variable valuation is a function $\bval \colon \BVar \in \{True, False\}$.

\emph{Discrete variables} are rational-valued, integer-valued or boolean-valued variables.
A set of discrete variables is $\DVar = \RVar \cup \IVar \cup \BVar $;
...
Let $\ArithExprD$ denote the set of arithmetic expressions over the numerical discrete variables, \ie{} made of addition, subtraction, multiplication, and division over rational (or integer) constants and discrete numerical variables.
...
Since version 3.1 \imitator{} support typed discrete variables. Few common primitive types as integer, rational and boolean are available. They can be declared with the following keywords :

\begin{itemize}
\item Rational: \styleIMI{discrete} or \styleIMI{rational}
\item Integer (32 bits): \styleIMI{int}
\item Boolean: \styleIMI{bool}
\end{itemize}

Theses types are divided into two categories: numerical and boolean. Integer and rational types belong to numerical category, while boolean type belong to boolean category.

\imitator{} models are statically typed in the sense of their variables and expressions types are resolved upstream of the algorithms execution.

In this continuity, discrete variables are not latently typed. The types are associated with variables and not with values. In other terms, once a variable is declared with a defined type, it only accept this type.

\subsection{Type checking}

In order to verify the model type consistency, a step called type checking, is applied - during the semantic analysis - on all expressions of the model: declarations, assignments, updates, conditions (\styleIMI{if-then-else}), guards and invariants. Type checking step do the following :

\begin{itemize}
 \item It check type consistency within the expressions
 \item It resolve the types of the expressions
 \item It apply a numerical type to literal numbers according to the numerical type holding by their outer expression
 \item It check consistency between an expression type and variable type, in case of assignment
 \item It check consistency between an expression type and it's grammar rule
\end{itemize}

\subsection{Expression type solving}

The expression tree is traversed (depth-first search) in order to deduce it's type. As soon as a variable, constant or literal is found, the expression take it's type. If the expression only contain literals numbers, it will be deduce as a rational arithmetic expression. 
...
If an expression mix different types, a type error is raise


\subsection{Literal number type inference}

The types of literal numbers are automatically resolved during the semantic analysis and more precisely during the type checking step. Once an expression type is resolved, all literal numbers within are automatically typed as the numerical type holding by the expression. 
...
As said previously, there is no implicit conversion of variables, constants or literals from one type to another type. All expressions are holding exactly one type and cannot mix different types. However, it's possible to convert an expression to another type using built-in conversion functions:

\begin{itemize}
\item \styleIMI{rational\_of\_int}: convert an int expression to a rational one
\end{itemize}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Discrete rational variables}\label{section:discrete}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Discrete rational variables\footnote{%
...
Discrete rational variables must be initialized to a single constant value in the \styleIMI{init} definition;
if they are not, a warning is issued, and they are arbitrarily set to~0.

Discrete rational variables can be tested in guards, and updated along transitions.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Discrete integer variables}\label{section:discrete_int}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Discrete integer variables are global integer-valued variables. As the same manner as discrete rational variables, their value is global. Discrete integer variables are encoded by a 32 bits signed integer and is architecture independent. The range of it's values is \styleIMI{[-2147483648, 2147483647]}. Integer are exact but bounded, so overflow can occur. In this case, \imitator{} can have an unspecified behavior.

Discrete integer variables must be initialized to a single constant expression or value, of the same type, in the init discrete definition; if they are not, a warning is issued, and they are arbitrarily set to 0.

As well as discrete rational variables, discrete integer variables can be tested in guards, and updated along transitions.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Discrete boolean variables}\label{section:discrete_boolean}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Discrete boolean variables are global boolean-valued variables. As the same manner as discrete rational variables, their value is global. Discrete boolean variables are encoded by two values : \styleIMI{True} or \styleIMI{False}.

Discrete boolean variables must be initialized to a single constant boolean expression or boolean value in the init discrete definition; if they are not, a warning is issued, and they are arbitrarily set to False.

As well as discrete rational variables, discrete boolean variables can be tested in guards, and updated along transitions.
...
The options \styleOption{-dynamic-elimination} (not activated by default) and \styleOption{-inclusion-bidir} can also be used to (try to) reduce the state space.
...
\begin{remark}\label{remark:accepting}
	...
	\styleIMI{property := \#witness EF(accepting);}

	or even:

	\styleIMI{property := \#synth EF(loc[pta] = l1 | (accepting \& loc[pta] <> l2));}
...
%
% 	\styleIMI{property := \#synth EF(False);}
%
% 	is absolutely equivalent to
%
% 	\styleIMI{property := \#synth EF(accepting);}.
%
...
\styleIMI{property := \#synth pattern(<PROP>)}
\\
...
\styleIMI{property := \#synth CycleThrough(state\_predicate);}
...
% The property syntax is as follows:
%
% \styleIMI{property := \#synth inf\_acc\_cycle;}
%
...
%
% \styleCommand{\binimitator{} model.imi property.imiprop -expl-order NDFS}
%
...
\styleIMI{property := \#synth Cycle;}


This is syntactic sugar for the following property:

\styleIMI{property := \#synth CycleThrough(True);}
%------------------------------------------------------------
\begin{syntaxalias}
...
\styleIMI{property := \#synth NZCycle;}
...
Since version ....., \imitator{} supports a translation of the model to \jani{} specification~\cite{BDHHJT17}.
To generate an equivalent \jani{} model without performing any analysis, use:

\styleCommand{\binimitator{} model.imi -imi2Jani}
...
\imitator{} and \jani{} do not have a same semantics to allow \styleIMI{if}-\styleIMI{then}-\styleIMI{else} assignments. In order to provide a same behavior, the translation is performed as follow.

\styleIMI{if (x=1) then (x:=2, y:=3) else (x=4)}\\ is transformed (and, after, written in \jani{} format) into
...
\paragraph{Stopwatched and multi-rate clocks}
If neither clock has a multi-rate (\ie{} there is no mention to clock rates in the model), all of them are declared in \jani{} model with the \styleIMI{clock} type.

Otherwise, if one rate is mentionned in the \imitator{} model, all of the clocks (including non multi-rate ones) are declared with the \styleIMI{continuous} type and the corresponding derivative (1 if non specified) is explicitly specified in each location.

\paragraph{Synchronization}
\jani{} synchronization semantics is more permissive than the \imitator{} one, therefore the exact meaning can be kept. It is done by listing synchronization vectors in the \styleIMI{syncs} entry.

\paragraph{Properties}
Properties are so far not translated to \jani{}. The corresponding input in the \jani{} file exists but remains empty.
...
% %------------------------------------------------------------
% \paragraph{\styleOption{-imi2GrML} (default: disabled)}
% %------------------------------------------------------------
