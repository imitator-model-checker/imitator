...
\end{enumerate}
In addition, \emph{arrays} and \emph{lists} (of elementary types, or of other arrays or lists) can be used.
...
Rational-valued variables can be directly compared to clocks and parameters within linear constraints.
...
Int-valued variables (nor the upcoming Boolean and binary word variables) \emph{cannot} be compared to clocks and parameters within linear constraints.
They can however be compared with other variables of compatible type.
...
An array variable always has a fixed length and an underlying type, which can be any of the available (discrete) types of \imitator{} (including arrays and lists, but excluding clock and parameters).
...
Lists are collections of dynamic length.
Contrarily to arrays, the type of a list does not depend on its length; it is thus possible to compare two lists of different lengths.
List elements can only be accessed---not updated---using an integer-valued arithmetic expression as index.
A list always has an underlying type, which can be any of the available (discrete) types of \imitator{} (including arrays and lists, but excluding clock and parameters).
For example, it is possible to declare a list of integers, of Booleans, of list of list of Booleans, etc.
...
\imitator{} 3.1 and higher comes with some built-in functions over discrete variables, summarized in \cref{table:summary:builtin-functions} and described in the following.
