...
In a \NIPTA{}, a clock~$\clock$ is stopped in a location $(\loc_1, \dots, \loc_N)$ if it stopped in at least one of the locations~$\loc_i$, \ie{} $\clock \in \stopwatches_i(\loc_i)$ for some~$i \in \{ 1 , \dots , N \}$.
...
\begin{remark}\label{remark:accepting}
	...
	In that case, the property becomes simply:
	
	\styleIMI{property := \#synth EF(accepting);}
	
	The semantics of the acceptance of a state is as follows: ``at least one of the locations is accepting (as specified by the \styleIMI{accepting} keyword).''
...
\styleIMI{property := \#synth cycle\_through(state\_predicate);}

%------------------------------------------------------------
\begin{syntaxalias}
	\imitator{} accepts the following equivalent keywords for \styleIMI{cycle\_through}:
	\begin{itemize}
		\item \styleIMI{cyclethrough}
		\item \styleIMI{loop\_through}
		\item \styleIMI{loopthrough}
	\end{itemize}
\end{syntaxalias}
%------------------------------------------------------------
...
% 	\item[\styleOption{-explOrder layerNDFSsub}]: NDFS with subsumption and layers (default)	;
...
\styleOption{-cycleAlgo NDFS}

It is also called when no \styleOption{-cycleAlgo} is specified.

Two additional options can be specified (a third one, \styleOption{-pendingOrder}, is explained later):
\begin{enumerate}
...
\styleIMI{property := \#synth cycle\_through(True);}
%------------------------------------------------------------
\begin{syntaxalias}
...
% 	\styleIMI{property := \#synth NZ\_cycle\_CUB;}
...
\begin{enumerate}
	\item a partial method (but slightly faster), that first detects whether the input \NIPTA{} is already a CUB-PTA; if so, it applies non-Zeno checking on this CUB-PTA; otherwise, the returned constraint will be false.
	      ...
	      \styleIMI{property := \#synth NZ\_cycle\_check;}
	...
	\item a complete method (though of course without guarantee of termination), that transforms the \NIPTA{} into a network of CUB-PTAs, and applies non-Zeno checking on the transformed CUB-PTA.
	      ...
	      \styleIMI{property := \#synth NZ\_cycle\_transform;}
...
\begin{table}[h!]
	...
	{\centering
		\begin{tabular}{ | l | l | c | c | }
			...
			\hline
			Accepting cycle        & \styleIMI{cycle\_through(state\_predicate)} & \cellYes{} & \cellYes{}  \\
			\hline
			Non-Zeno cycles        & \styleIMI{NZ\_cycle}                            & \cellYes{} & \cellNo{}  \\
...
%------------------------------------------------------------
\paragraph{\styleOption{-cycleAlgo} (default: NDFS)}
%------------------------------------------------------------
For cycle synthesis algorithms (\cref{ss:mode:LoopSynth}), call either \styleOption{Loop} (BFS) or \styleOption{NDFS}, with some additional options in the latter case.
See
	\styleOption{-layer},
	\styleOption{-subsumption},
	\styleOption{-pendingOrder},
...
In reachability algorithms (\cref{ss:mode:EFsynth,ss:mode:EF,ss:mode:EFmin,ss:mode:EFmax,ss:mode:EFopt,ss:mode:prop,ss:mode:PRP}), \styleOption{-inclusion} is usually enabled by default; however, for loop synthesis algorithms (\cref{ss:mode:LoopSynth,ss:mode:Zeno}), it is disabled as it may introduce spurious cycles.
% TODO: table summarizing everything
...
%------------------------------------------------------------
\paragraph{\styleOption{-inclusion-bidir} (default: disabled)}
%------------------------------------------------------------
...
%------------------------------------------------------------
\paragraph{\styleOption{-no-inclusion-test-in-EF} (default: disabled)}
%------------------------------------------------------------
...
%------------------------------------------------------------
For the cycle detection using NDFS (\cref{sss:accepting-loop-NDFS}),
\styleOption{-step} is used for iterative NDFS: it indicates the step between two iterations.
...
%------------------------------------------------------------
\paragraph{\styleOption{-tree} (default: disabled)}
%------------------------------------------------------------
Does not test if a new state was already encountered.
...
\begin{tabular}{l l}
	...
	\\
% 	$|$ & \styleIMI{inf\_acc\_cycle}                                                                                           \\
	$|$ & \styleIMI{cycle\_through} \nt{state\_predicate}
	\\
	% NOTE: alias
	$|$ & \styleIMI{cyclethrough} \nt{state\_predicate}
		\commentgrammar{alias for \styleIMI{cycle\_through}}
	\\
	$|$ & \styleIMI{loop\_through} \nt{state\_predicate} 
		\commentgrammar{alias for \styleIMI{cycle\_through}}
	\\
	$|$ & \styleIMI{loopthrough} \nt{state\_predicate}
		\commentgrammar{alias for \styleIMI{cycle\_through}}
	\\
	$|$ & \styleIMI{NZ\_cycle\_check}                                                                                     \\
	$|$ & \styleIMI{NZ\_cycle\_transform}                                                                                 \\
	% NOTE: hidden for sake of simplicity
	% 	$|$  & \styleIMI{NZ\_cycle\_CUB} \\
