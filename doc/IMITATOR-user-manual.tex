...
if they are not, a warning is issued, and they are arbitrarily set to a standard value (\eg{}~ \styleIMI{0} for numerical variables, \BFalse{} for Boolean variables, and to \styleIMI{0b0} for Binary words).
...
Binary word variables are binary strings of fixed length.
...
\begin{itemize}
    \item \styleIMI{rational\_of\_int(x : int) : rational} : convert a int number to a rational one
\end{itemize}

\subsection{Binary word functions}
...
Theses types are divided into three categories: numerical (for \styleIMI{rational} and \styleIMI{int}), Booleans (for \styleIMI{bool}) and Binary (for \styleIMI{binary(l)}).
