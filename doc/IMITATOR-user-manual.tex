...
A user-defined function is made of a (possibly empty) list of declarations of local variables, of a set of imperative instructions (modification of global variables) and must have one return expression.
...
\begin{itemize}
	...
	\item Shadowing a global variable with a local one is possible (local variable with the same name as a global variable);
	\item Shadowing a local variable with another local variable is possible (local variable with the same name as a previously declared local variable);
