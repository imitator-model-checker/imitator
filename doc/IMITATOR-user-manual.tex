...
Users have the ability to declare their own functions, called user defined functions.

%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%
\subsection{Declaration}
%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%

A user defined function can be compound of declarations (local variables), imperative instructions (modification of global variables) and must have one return expression.

User defined functions can have zero or many parameters and must return one value. Note that all function parameters are passed by value.

User functions must be declared directly after the variable declarations and are global to all \IPTA.

\paragraph{} Below, some user function declarations examples :
...
User defined functions can be called in guards, invariants or updates as the same manner as the built-in functions.
Of course, side-effect functions cannot be used anywhere than in sequential updates.
...
\begin{IMITATORmodel}
	when two() = 2 goto lend;
	when add_one(5) = 6 goto lend;
	when add(1, 2) = 3 goto lend;
...
As Jani doesn't seems to have a scheme that enable the declaration of local variables in user defined functions, for the moment, only the user defined functions containing one expression as a body are correctly translated.
