...
\begin{remark}
	...
	Side-effects functions can only be used in updates, more precisely in the body of the \styleIMI{let} block of an update (see \cref{ss:updates}).
\end{remark}
...
%------------------------------------------------------------
\subsubsection{\styleIMI{do-then} updates}\label{section:do_then_form_updates}
%------------------------------------------------------------

\styleIMI{do-then} updates are more complex updates, allowing first a set of sequential updates (on the discrete variables only) called the \styleIMI{do} block, followed by ``standard updates'' (including side-effects functions), called the \styleIMI{then} block.

\paragraph{The \styleIMI{do} block}

A \styleIMI{do} block can be used to define sequential updates on any discrete variable of any type.
Contrary to the \styleIMI{then} block (or to the standard updates), it is possible to make modifications of data structures (such as stacks and queues) with side-effects functions.
The \styleIMI{do} block can be seen as an imperative update part that is processed before the \styleIMI{then} block.
It is not possible to update clocks in a \styleIMI{do} block.
...
\begin{IMITATORmodel}
	...
	(* Now, j = 1, while i = 3 OR i = 4 (unspecified!) *)
\end{IMITATORmodel}
...
\paragraph{The \styleIMI{do-then} block}

\begin{remark}
For a given transition, \styleIMI{do} block is executed before the \styleIMI{then} block.
\end{remark}

\begin{remark}
In a model that contain multiple synchronized \IPTA{}, all \styleIMI{do} blocks (all sequential updates) in all PTAs are processed first, following the definition order of PTAs.
Only once after all \styleIMI{do} blocks are computed, all \styleIMI{then} block are updated. Contrary to \styleIMI{do} blocks, keep in mind that updates contained in \styleIMI{then} blocks does not be computed sequentially.
\end{remark}

\begin{remark}
Note the use of semicolon in the sequential part (\styleIMI{do} block) and comma in the declarative part (\styleIMI{then} block).
\end{remark}
...
\begin{IMITATORmodel}
	do {
		...
		do
			...
			i := i + 1; (* now, i = 1 *)
		then
			j := i (* as then block is executed after do block, j = 1 *)
		end
