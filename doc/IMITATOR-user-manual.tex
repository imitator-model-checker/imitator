...
State predicates allow conjunction, disjunction and the use of global discrete variables.
...
\imitator{} can also
	output the trace set in a graphical form (option \styleOption{-draw-statespace}),
	output the constraint synthesized in a graphical form in two dimensions (option \styleOption{-draw-cart}).
It also
	outputs the result to a text file using a normalized syntax% (option \styleOption{-output-result})
	, that can be then scripted from external programs.
...
\styleIMI{property := \#synth IM(referencevaluation);}

\noindent{}
where \styleIMI{referencevaluation} is a reference valuation of the form \styleIMI{p1 = 1 \& p2 = 2 \&…} (see \cref{section:grammar-property} for details).
...
\styleIMI{property := \#synth BCcover(referencerectangle);}

\noindent{}
where \styleIMI{referencerectangle} is a reference hyper-rectangle of the form \styleIMI{p1 = 1..5 \& p2 = 2..4 \&…} (see \cref{section:grammar-property} for details).


\imitator{} can also
	output all trace sets in a graphical form (option \styleOption{-draw-statespace}),
	or
...
In addition, trace sets and separate graphical cartographies for each tile can also be generated by adding option \styleOption{-tiles-files}.
...
\styleIMI{property := \#synth BCrandom(referencerectangle , XX);}


\noindent{}where \styleOption{XX} is the number of times an integer point is randomly selected within the domain defined.
If this point is already covered by one of the tiles, the inverse method is not called, an another point is selected.
Note that \styleOption{XX} represents the number of integer points randomly selected; the number of \emph{calls} to the inverse method can be significantly smaller.
...
\styleIMI{property := \#synth BCshuffle(referencerectangle);}
...
\styleIMI{property := \#synth PRP(statepredicate , referencevaluation);}
...
\styleIMI{property := \#synth PRPC(statepredicate , rectanglevaluation);}
...
\imitator{} can also output the trace set in a graphical form using option \styleOption{-draw-statespace}.
% TODO: example
...
\styleCommand{\binimitator{} model.imi [options] -draw-statespace normal}
...
Using \styleOption{-draw-statespace light} makes a more compact representation, but is also less informative as only the state number and the transition labels are shown.
...
\begin{example}
	An example of state space with no details (output using \styleOption{-draw-statespace light}), with normal details (using \styleOption{-draw-statespace normal}), and in verbose mode (using \styleOption{-draw-statespace full}) are given in \cref{figure:traceset:nodetail,figure:traceset:normal,figure:traceset:verbose}.

	These graphics were generated using the example in \cref{chapter:syntax-introduction} with the following command:

	\styleCommand{imitator fischer.imi -mode EF -depth-limit 4 -draw-statespace normal}

	(where \styleOption{normal} should be replaced with \styleOption{light} or \styleOption{full}, respectively)
\end{example}
...
\styleCommand{\binimitator{} model.imi [options] -draw-cart}
...
platforms.
The generated files will be located in the current directory, unless option \styleOption{-output-files-prefix} is used.
...
%------------------------------------------------------------
\paragraph{\styleOption{-draw-trace-set} (default: none)}
%------------------------------------------------------------
...
\begin{itemize}
	\item Use value \styleOption{light} for the structure only (no location names): does \emph{not} provide detailed information on the local locations of the composed \IPTA{}, and instead only outputs the state id.
	Enabling the option with this value instead of \styleOption{-draw-trace-set normal} may yield a smaller graph, which is useful when generating relatively large state spaces.
...
Enabling the option with this value instead of \styleOption{-draw-trace-set normal} will yield a very large graph, and it is useful (and readable) mostly for very small state spaces.
...
%------------------------------------------------------------
Consider an inclusion of parametric zone instead of the equality when performing the $\mathit{Post}$ operation.
...
However, when the \code{-inclusion} option is activated, it suffices that a previous state with the same location and a constraint \emph{greater than or equal} to the constraint of the new state has been encountered to discard the new state and stop exploring the current branch.
...
%------------------------------------------------------------
\paragraph{\styleOption{-output-files-prefix} (default: \stylePath{<input\_file>})}
%------------------------------------------------------------
...
Examples:
\begin{itemize}
	\item \styleOption{-output-files-prefix log}
	\item \styleOption{-output-files-prefix ./log}
	\item \styleOption{-output-files-prefix /home/imitator/outputs}
\end{itemize}
...
%------------------------------------------------------------
\paragraph{\styleOption{-no-result-file} (default: false)}
%------------------------------------------------------------
