...
Each state is represented with its discrete part (location and values of the discrete variables), the clock and parameter constraints, and below their projection onto the parameters; in addition, if \styleFileContent{projectresult} is defined in the model (see \cref{section:projection}), the projection onto a specific set of parameters is also added.
...
In addition, the parameter to be minimized must be specified at the end of the model as follows:

\styleIMI{minimize(parameter\_name)}\\

The standard command is:

\styleCommand{\binimitator{} model.imi -mode EFmin -merge -incl -output-result}
...
The parameter to be maximized must be specified at the end of the model as follows:

\styleIMI{maximize(parameter\_name)}\\

The standard command is:

\styleCommand{\binimitator{} model.imi -mode EFmax -merge -incl -output-result}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{EF with optimal time reachability}\label{ss:mode:EFopt}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Parameter synthesis using properties}\label{ss:mode:prop}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
The accepting location is defined as usual using the following syntax:

\begin{center}
	\styleIMI{property := unreachable loc[pta] = …;}
\end{center}

…which can be misleading as the property is defined using \styleIMI{unreachable} while the location should actually be reachable.
Patterns (see \cref{ss:mode:prop}) can also be used. % NOTE: not tested

The command for accepting cycle synthesis is:

\styleCommand{\binimitator{} model.imi -mode AccLoopSynth -output-result}
...
\styleCommand{\binimitator{} model.imi -mode AccLoopNDFS -output-result}
...
%------------------------------------------------------------
For algorithm \EFsynth{}, stops the analysis as soon as one target state is found, and returns the (possibly approximated) result.
...
% 		(see \cref{ss:mode:statespace}) %TODO?
	...
	\styleOption{EF} & Parametric reachability synthesis (\EFsynth{} \cite{JLR15}) \\
