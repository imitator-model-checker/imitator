...
An array variable always has an underlying type, which can be any of available types of \imitator{}.
For example, it's possible to declare an array of int, an array of bool, an array of array of bool - an array can be multi-dimensional - etc. following the syntax below:
...
\begin{itemize}
    \item \styleIMI{pow(x : 'a, e : int) : 'a} with \styleIMI{'a : [rational | int]}: compute the power of a number
\end{itemize}
...
\begin{itemize}
	\item \styleIMI{array\_concat(a1 : 'a array(l1), a2 : 'a array(l2)) : 'a array(l1 + l2)} with \styleIMI{'a : any}: concat an array of length \styleIMI{l1} with an array of length \styleIMI{l2}, resulting to an array of length \styleIMI{l1 + l2}
\end{itemize}

%------------------------------------------------------------
\begin{example}
	To illustrate the arrays operations, note that all the following guards in the model fragment below will evaluate to true:

%------------------------------------------------------------
	\begin{IMITATORmodel}
		when array_concat([1, 2, 3] , [4, 5, 6])     = [1, 2, 3, 4, 5, 6]    goto l1;
		when array_concat([[1, 2]] , [[3, 4]])     = [[1, 2], [3, 4]]    goto l2;
	\end{IMITATORmodel}
%------------------------------------------------------------
...
The set of discrete variables is $\DVar = \RVar \cup \IVar \cup \BVar \cup \WVar \cup \AVar $.
...
\begin{becareful}
	Although it's possible in \imitator{} to comparing an array variable to a literal array or literal array to another one, this type of comparison isn't supported by \uppaal{}.

\end{becareful}

\begin{becareful}
	Built-in functions operating on array are not supported by our translation.
\end{becareful}
...
\begin{becareful}
	Unsupported \imitator{} built-in functions will not be declared (and implemented) in \uppaal{} translation, resulting to a bad program when trying to call one. The declaration and implementation of these functions in \uppaal{}, if the \uppaal{} formalism allow it, is the responsability of the user.
\end{becareful}
...
The syntax going beyond that of \hytech{} (typically, booleans, binary words, array…) and \imitator{} built-in functions
...
\begin{tabular}{l l}
	\   & \styleIMI{[} \styleIMI{]} \\
	$|$ & \styleIMI{[} \nt{literal\_array\_fol} \styleIMI{]} \\
\end{tabular}

%------------------------------------------------------------
\regleGrammaire{literal\_array\_fol}
%------------------------------------------------------------
\begin{tabular}{l l}
	\   & \nt{global\_expression} \styleIMI{,} \nt{literal\_array\_fol} \\
	$|$ & \nt{global\_expression} \\
\end{tabular}
