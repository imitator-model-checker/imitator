...
\imitator{} is a software tool to perform automated parameter synthesis for concurrent timed systems.
...
``given a concurrent timed system, what are the values of the timing constants that guarantee that the system satisfies some property?''
...
\imitator{} is a command-line only tool, but that outputs results in graphical form, when applicable.
...
% TODO
% TODO: say no GUI, but refer to CosyVerif	
...
% TODO: add graphic representation, describe model, make at least one call
...
%TODO: rules for VARIABLE NAMES !! (and multiple names as multiple objects (e.g., same location names in different IPTA))
