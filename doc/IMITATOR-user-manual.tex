...
% This new section introduce a more powerful initialization system allowing initialization of discrete variables by constant expressions. It mean that it's possible to initialize any discrete variables of any type (proposed by \imitator{}) with an expression that can be resolved at compile time (use of variables is forbbiden). It's also the only way to initialize discrete integer and discrete boolean variables.
...
if they are not, a warning is issued, and they are arbitrarily set to a standard value (\eg{} \styleIMI{0} for numerical variables, or \BFalse{} for Boolean variables; see \cref{section:init} for the actual value).

Discrete variables can be tested in guards, and updated along transitions.

\imitator{} currently supports four elementary types of discrete variables:
\begin{enumerate}
...
\end{enumerate}
In addition, \emph{arrays} (of elementary types, or of arrays) can be used.
...
Lists are collections of variable length - contrary to arrays, the type of a list doesn't depend on it's length, thus it's possible to compare two lists of different lengths.
...
For example, it is possible to declare a list of integers, of Booleans, of list of list of Booleans---which can be seen as a multidimensional list.
...
Stacks are LIFO collections (Last In First Out) of variable length - contrary to arrays, the type of a stack doesn't depend on it's length, thus it's possible to compare two stacks of different lengths.
...
For example, it is possible to declare a stack of integers, of Booleans, of stacks of Booleans---which can be seen as a multidimensional stack.
\begin{remark}
...
\begin{IMITATORmodel}
	...
	my_2d_int_stack      	: int stack stack;
\end{IMITATORmodel}
%------------------------------------------------------------
...
Queues are FIFO collections (First In First Out) of variable length - contrary to arrays, the type of a queue doesn't depend on it's length, thus it's possible to compare two queues of different lengths.
...
For example, it is possible to declare a queue of integers, of Booleans, of queues of Booleans---which can be seen as a multidimensional queue.
\begin{remark}
...
\begin{IMITATORmodel}
	...
	my_2d_int_queue      	: int queue queue;
\end{IMITATORmodel}
%------------------------------------------------------------
...
\begin{remark}
Some functions, mistakenly called "Side-effects functions" allow the modification of collections "in-place" and therefore are subject to side-effects.
Side-effects functions can only be used in updates, more precisely in the body of the \styleIMI{let} bloc of an update.
\end{remark}

\paragraph{Arithmetic functions}

\begin{itemize}
    \item \label{item:lbl-pow} \styleIMI{pow(x:'a number, e:int):'a with 'a number:[rational|int]} - compute the power of a number
\end{itemize}

%------------------------------------------------------------
\begin{example}
	To illustrate above operation, note that all the following guards in the model fragment below will evaluate to true:
...
%------------------------------------------------------------
	\begin{IMITATORmodel}
		(* i : int *)
		(* r : rational *)
		when rational_of_int(i) = r goto l1;
	\end{IMITATORmodel}
%------------------------------------------------------------
\end{example}
%------------------------------------------------------------
...
\begin{itemize}
	\item \styleIMI{a[i:int]:'a with 'a:any} - access to element of array \styleIMI{a} at index \styleIMI{i}
	\item \label{item:lbl-array_append} \styleIMI{array\_append(a1:'a array(l1), a2:'a array(l2)):'a array(l1+l2) with 'a:any} - concatenates an array of length \styleIMI{l1} with an array of length \styleIMI{l2}, resulting in an array of length \styleIMI{l1 + l2}
	\item \label{item:lbl-array_length} \styleIMI{array\_length(a1:'a array(l1)):int=l1 with 'a:any} - length of the array \styleIMI{a1}
	\item \label{item:lbl-array_mem} \styleIMI{array\_mem(a1:'a, l:'a array(l1)):bool with 'a:any} - true if \styleIMI{e} is in the the array \styleIMI{a1}
\end{itemize}
...
\begin{itemize}
	\item \styleIMI{l[i:int]:'a with 'a:any} - access to element of list \styleIMI{l} at index \styleIMI{i}
	\item \label{item:lbl-list_cons} \styleIMI{list\_cons(e:'a, l:'a list):'a list with 'a:any} - construct a list by inserting a new element \styleIMI{e} at the beginning of \styleIMI{l}
	\item \label{item:lbl-list_hd} \styleIMI{list\_hd(l:'a list):'a with 'a:any} - head element of a list \styleIMI{l}
	\item \label{item:lbl-list_is_empty} \styleIMI{list\_is\_empty(l:'a list):bool with 'a:any} - Check if the list \styleIMI{l} has elements
	\item \label{item:lbl-list_length} \styleIMI{list\_length(l:'a list): int with 'a:any} - length of the list \styleIMI{l}
	\item \label{item:lbl-list_mem} \styleIMI{list\_mem(e:'a, l:'a list):bool with 'a:any} - true if \styleIMI{e} is in the the list \styleIMI{l}
	\item \label{item:lbl-list_rev} \styleIMI{list\_rev(l:'a list):'a list with 'a:any} - reverse the the list \styleIMI{l}
	\item \label{item:lbl-list_tl} \styleIMI{list\_tl(l:'a list):'a list with 'a:any} - tail of the list \styleIMI{l}
\end{itemize}
...
\begin{itemize}
	\item \label{item:lbl-stack_push} \styleIMI{stack\_push(e:'a, s:'a stack):'a stack with 'a:any} - Add an element \styleIMI{e} at the top of the stack \styleIMI{s} and return the stack - the stack is modified, this function is subject to side-effects
	\item \label{item:lbl-stack_pop} \styleIMI{stack\_pop(s:'a stack):'a with 'a:any} - Get and remove the element at the top of the stack \styleIMI{s} - the stack is modified, this function is subject to side-effects
	\item \label{item:lbl-stack_top} \styleIMI{stack\_top(s:'a stack):'a with 'a:any} - Get the element at the top of the stack \styleIMI{s}
	\item \label{item:lbl-stack_clear} \styleIMI{stack\_clear(s:'a stack):'a stack with 'a:any} - Remove all elements of the stack \styleIMI{s} and return the stack - the stack is modified, this function is subject to side-effects
	\item \label{item:lbl-stack_is_empty} \styleIMI{stack\_is\_empty(s:'a stack):bool with 'a:any} - Check if the stack \styleIMI{s} has elements
	\item \label{item:lbl-stack_length} \styleIMI{stack\_length(s:'a stack):int with 'a:any} - Get the number of elements in the stack \styleIMI{s}
\end{itemize}

%------------------------------------------------------------
\begin{example}
	To illustrate the stacks operations, note that the following guard in the model fragment below will evaluate to true:
...
\begin{itemize}
	\item \label{item:lbl-queue_push} \styleIMI{queue\_push(e:'a, q:'a queue):'a queue with 'a:any} - Add an element \styleIMI{e} at the beginning of the queue \styleIMI{q} and return the queue - the queue is modified, this function is subject to side-effects
	\item \label{item:lbl-queue_pop} \styleIMI{queue\_pop(q:'a queue):'a with 'a:any} - Get and remove the last element of the queue \styleIMI{q} - the queue is modified, this function is subject to side-effects
	\item \label{item:lbl-queue_top} \styleIMI{queue\_top(q:'a queue):'a with 'a:any} - Get the last element of the queue \styleIMI{q}
	\item \label{item:lbl-queue_clear} \styleIMI{queue\_clear(q:'a queue):'a queue with 'a:any} - Remove all elements of the queue \styleIMI{q} and return the queue - the queue is modified, this function is subject to side-effects
	\item \label{item:lbl-queue_is_empty} \styleIMI{queue\_is\_empty(q:'a queue):bool with 'a:any} - Check if the queue \styleIMI{q} has elements
	\item \label{item:lbl-queue_length} \styleIMI{queue\_length(q:'a queue):int with 'a:any} - Get the number of elements in the queue \styleIMI{q}
\end{itemize}

%------------------------------------------------------------
\begin{example}
	To illustrate the queues operations, note that the following guard in the model fragment below will evaluate to true:
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Discrete variables updates}\label{section:discrete_boolean}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Standard updates}\label{section:standard_updates}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
For more information on how the standard updates works, read the section \styleIMI{do-then} form updates below.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{do-then form updates}\label{section:do_then_form_updates}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\styleIMI{do-then} form updates are more complex updates, allowing, at the one hand, sequential updates on discrete values, at the other hand
the use of side-effects functions for in-place modification of data structure values.
If you don't need these functionalities, we recommend the use the standard updates. \styleIMI{do-then} form updates are updates composed of two parts.

\subsubsection{do bloc}

\paragraph{}

\styleIMI{do} bloc can be used to make sequential updates on any discrete variables whatever their types.
Contrary to \styleIMI{then} bloc (or standard update) it's possible to make in-place modifications on data structures (like stacks and queues) with side-effects functions.
The \styleIMI{do} bloc can be seen as an imperative update part that is processed before the constraints updates (\styleIMI{then} bloc).
It's not possible to update clock constraint in \styleIMI{do} bloc because of the sequential behavior.

\paragraph{}
Examples of a \styleIMI{do} update expression:
...
\begin{IMITATORmodel}
	do {
		...
		(* only do expression *)
		do
			(* in-place modification of stack s *)
			stack_push(1, s);
			(* in-place modification of stack s *)
			stack_push(2, s);
		end
...
\subsubsection{then bloc}

\paragraph{}

\styleIMI{then} bloc can be used to update clock(s) or discrete variable(s) constraints.
Update of a discrete variable constraint is equivalent to an assignment. However, these updates are not sequentials,
so as described before, in case of multiple update of one variable, a warning is issued, and the behavior of the \NIPTA{} becomes unspecified.
\styleIMI{then} bloc is the same as a standard update, see \cref{section:standard_updates}.

\paragraph{}
An example of a \styleIMI{then} update expression:
...
\begin{IMITATORmodel}
	...
	do {
		...
		i := 1; (* i = 1 *)
		(* multiple update of i below, it doesn't take into account *)
		(* i will still be equal to 1 *)
		i := i + 1;
		(* below j = i, as initial value of i = 0 for this state, j = 0 *)
		(* keep in mind that it's not a sequential update *)
...
\subsubsection{do-then bloc}

\begin{remark}
For a given transition, \styleIMI{do} bloc is executed before the \styleIMI{then} bloc.
\end{remark}

\begin{remark}
In a model that contain multiple synchronized \IPTA{}, all \styleIMI{do} blocs (all sequential updates) in all PTAs are processed first, following the definition order of PTAs.
Only once after all \styleIMI{do} blocs are computed, all \styleIMI{then} bloc are updated. Contrary to \styleIMI{do} blocs, keep in mind that updates contained in \styleIMI{then} blocs doesn't be computed sequentially.
\end{remark}

\begin{remark}
Note the use of semicolon in the sequential part (\styleIMI{do} bloc) and comma in the declarative part (\styleIMI{then} bloc).
\end{remark}

\paragraph{}
Examples of a \styleIMI{do-then} update expression:
...
\begin{IMITATORmodel}
	do {
		...
		do
			...
			i := i + 1; (* now, i = 1 *)
		then
			j := i (* as then bloc is executed after do bloc, j = 1 *)
		end
...
\begin{IMITATORmodel}
	do {
		...
		do
			...
			(* i = 1, below *)
			i := i + 1;
			(* below, updating clock constraint is forbidden *)
			(* x := 0 *)
		then
			x := i,
			y := stack_top(s),
			...
			r := 0,
			r := 1
			(* below, using side-effects function is forbidden *)
			(* r := stack_pop(s) *)
		end
...
This initial default value is as follows, depending on the variable type:
...
The initial default value of arrays and lists is that of their type.
...
% %------------------------------------------------------------
% \begin{terminal}
% imitator model.imi -mode EFunsafe -merge -incl -output-result
% \end{terminal}
%------------------------------------------------------------
...
The options \styleOption{-merge} and \styleOption{-comparison inclusion} are implicitly activated for this algorithm, as they usually greatly increase the analysis efficiency and the termination.
...
% %------------------------------------------------------------
% \begin{terminal}
%imitator model.imi -mode EF -merge -incl -output-result
% \end{terminal}
%------------------------------------------------------------
...
% This algorithm differs from the one in \cref{sss:accepting-loop-BFS} in the sense that this algorithm performs a nested depth-first search (NDFS) exploration until a cycle is found~\cite{NPP18}.
% In addition, this algorithm is (by default) not complete.
%
% The property syntax is as follows:
%
% \begin{IMITATORproperty}
% property := #synth inf_acc_cycle;
% \end{IMITATORproperty}
%
% However, there are two main differences with \cref{ss:loop,sss:accepting-loop-BFS}:
% \begin{enumerate}
...
% %------------------------------------------------------------
% \begin{terminal}
% imitator model.imi property.imiprop -expl-order NDFS
% \end{terminal}
%------------------------------------------------------------
%
% Several heuristics can be used, some of them synthesizing all parameter values for which a cycle can be found.
% These are accessible via the \styleOption{-expl-order} option:
%
% \begin{description}
% 	\item[\styleOption{-expl-order NDFS}]: use standard NDFS to find an accepting cycle;
% 	\item[\styleOption{-expl-order NDFSsub}]: standard NDFS with subsumption;
% 	\item[\styleOption{-expl-order layerNDFS}]: NDFS with layers;
% 	\item[\styleOption{-expl-order layerNDFSsub}]: NDFS with subsumption and layers (default)	;
...
The syntax going beyond that of \hytech{} (typically, Booleans, binary words, arrays, lists…)\ and \imitator{} built-in functions
is obviously not translated correctly.
...
For example, in the following update expression \styleIMI{b := True \& i < 3 + 1} with \styleIMI{i : int}, literal numbers \styleIMI{3} and \styleIMI{1} will be automatically typed as a \styleIMI{int}.
...
%First,
for standard reachability analysis (and counterexample analysis, using \styleIMI{\#witness}), and are documented in~\cite{ANP17}.
\begin{itemize}
...
%------------------------------------------------------------
Clock extrapolation was defined in a number of papers for timed automata (\eg{} \cite{BBFL03,BBLP06,Li09,Tripakis09}), and then extended to parametric timed automata in~\cite{AA22}.
...
\begin{longtable}{@{} l @{\ \ } p{12cm}}
	\styleOption{reconstruct} & Update the statespace with a copy of the reachable part \\
	\styleOption{on-the-fly} & Update the statespace by deleting the merged state and updating its transitions \emph{in situ}
\end{longtable}


%------------------------------------------------------------
\paragraph{\styleOption{-merge none} (default: depending on the algorithm)}
%------------------------------------------------------------
...
%------------------------------------------------------------
Use the merging technique of \cite{AFS13atva} and specify the candidates to merge with a state.
...
%------------------------------------------------------------
Use the merging technique of \cite{AFS13atva} and specify when to update the statespace.
...
%------------------------------------------------------------
Use the merging technique of \cite{AFS13atva} and set if a state can be merged to one of its siblings, 
...
%------------------------------------------------------------
\begin{tabular}{l l}
	\  & \styleIMI{do} \styleIMI{ \{ } \nt{do\_then\_updates} \styleIMI{ \} } \\
\end{tabular}

%------------------------------------------------------------
\regleGrammaire{do\_then\_updates}
%------------------------------------------------------------
\begin{tabular}{l l}
...
%
Benjamin~Loillier significantly enhanced the syntax, notably allowing Boolean and int types, and new syntactic enhancements such as arrays.
%
