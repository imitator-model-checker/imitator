...
\lstdefinelanguage{Imitator}{
	morekeywords={
		and, automaton, clock, constant, discrete, do, end, endreach, False, forward, from, goto, if, in, init, initially, loc, locations, not, or, parameter, print, reach, region, unreachable, urgent, sync, synclabs, True, var, wait, when, while
	},
	sensitive=false,
...
\paragraphe{The Fischer mutual exclusion protocol}
...
This version of the protocol is neither the most complete, nor the most simple; we just use here to introduce various aspects of the \imitator{} input syntax.
...
\paragraphe{Input syntax}
We give below this example using the \imitator{} syntax.
...
\paragraphe{Variable declarations}
...
At any time, when the discrete variable \styleIMI{turn} is equal \styleIMI{IDLE}, then this \IPTA{} synchronizes on action \styleIMI{try\_1}, resets its clock \styleIMI{x1}, and enters location \styleIMI{active1}.
...
Then, at any time, this \IPTA{} synchronizes on action \styleIMI{update\_1}, resets its clock \styleIMI{x1} and sets the global variable \styleIMI{turn} to \styleIMI{1}, and enters location \styleIMI{check1}.
...
If \styleIMI{turn} is still equal to \styleIMI{1} (that is, no other process attempted in the meanwhile to enter the critical section), then process~1 indeed enters the critical section, by synchronizing \styleIMI{access\_1} and resetting \styleIMI{x1}.
...
\paragraphe{The second process}
Process~2 is symmetrical to process~1, except that \styleIMI{x1} is replaced with \styleIMI{x2}, and that the value of \styleIMI{turn} becomes~2.


\paragraphe{The observer}
The observer is in charge to observe that not two processes are in critical section at the same time.\footnote{%
...
Finally, parameters are bound to be positive or null (this is not assumed by default, so users are strongly advised to add this constraint).
...
\paragraphe{Property specification}


\paragraphe{Parameter synthesis}
