...
An array variable always has a fixed length and an underlying type, which can be any of the available types of \imitator{} (including arrays).
...
\begin{itemize}
	\item \styleIMI{array\_concat(a1 : 'a array(l1), a2 : 'a array(l2)) : 'a array(l1 + l2)} with \styleIMI{'a : any}: concatenates an array of length \styleIMI{l1} with an array of length \styleIMI{l2}, resulting in an array of length \styleIMI{l1 + l2}
\end{itemize}
...
(We do not formalize the arrays in the following, as we believe their usage is straightforward.)
...
The initial default value of arrays is that of their type.
...
The syntax going beyond that of \hytech{} (typically, Booleans, binary words, arrays…)\ and \imitator{} built-in functions
