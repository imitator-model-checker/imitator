...
For more information on how the standard updates works, read the section \styleIMI{let-in} form updates below.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Let-in form updates}\label{section:let_in_form_updates}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\styleIMI{let-in} form updates are more complex updates, allowing, at the one hand, sequential updates on discrete values, at the other hand
the use of side-effects functions for in-place modification of data structure values.
If you don't need these functionalities, we recommend the use the standard updates. \styleIMI{let-in} form updates are updates composed of two parts.

\subsubsection{Let bloc}

\paragraph{}

\styleIMI{let} bloc can be used to make sequential updates on any discrete variables whatever their types.
Contrary to \styleIMI{in} bloc (or standard update) it's possible to make in-place modifications on data structures (like stacks and queues) with side-effects functions.
The \styleIMI{let} bloc can be seen as an imperative update part that is processed before the constraints updates (\styleIMI{in} bloc).
It's not possible to update clock constraint in \styleIMI{let} bloc because of the sequential behavior.

\paragraph{}
Examples of a \styleIMI{let} update expression:
...
\subsubsection{In bloc}

\paragraph{}

\styleIMI{in} bloc can be used to update clock(s) or discrete variable(s) constraints.
...
\styleIMI{in} bloc is the same as a standard update, see \cref{section:standard_updates}.

\paragraph{}
An example of a \styleIMI{in} update expression:
...
\subsubsection{Let-in bloc}

\begin{remark}
For a given transition, \styleIMI{let} bloc is executed before the \styleIMI{in} bloc.
\end{remark}

\begin{remark}
In a model that contain multiple synchronized \IPTA{}, all \styleIMI{let} blocs (all sequential updates) in all PTAs are processed first, following the definition order of PTAs.
Only once after all \styleIMI{let} blocs are computed, all \styleIMI{in} bloc are updated. Contrary to \styleIMI{let} blocs, keep in mind that updates contained in \styleIMI{in} blocs doesn't be computed sequentially.
\end{remark}

\begin{remark}
Note the use of semicolon in the sequential part (\styleIMI{let} bloc) and comma in the declarative part (\styleIMI{in} bloc).
\end{remark}

\paragraph{}
Examples of a \styleIMI{let-in} update expression:
...
\begin{IMITATORmodel}
	do {
		...
		let
			...
			i := i + 1; (* now, i = 1 *)
		in
			j := i (* as in bloc is executed after let bloc, j = 1 *)
		end
...
%------------------------------------------------------------
\begin{tabular}{l l}
	\  & \styleIMI{do} \styleIMI{ \{ } \nt{let\_in\_updates} \styleIMI{ \} } \\
\end{tabular}

%------------------------------------------------------------
\regleGrammaire{let\_in\_updates}
%------------------------------------------------------------
\begin{tabular}{l l}
