...
\begin{itemize}
    \item \styleIMI{shift\_left(b : binary\_word(l), i : int) : binary\_word(l)}: shift a binary word of length \styleIMI{l} to left
		...
		\item \styleIMI{log\_and(b1 : binary\_word(l), b2 : binary\_word(l)) : binary\_word(l)}: make AND bitwise operation between two binary words of length \styleIMI{l}
		\item \styleIMI{log\_or(b1 : binary\_word(l), b2 : binary\_word(l)) : binary\_word(l)}: make OR bitwise operation between two binary words of length \styleIMI{l}
		\item \styleIMI{log\_xor(b1 : binary\_word(l), b2 : binary\_word(l)) : binary\_word(l)}: make XOR bitwise operation between two binary words of length \styleIMI{l}
		\item \styleIMI{log\_not(b : binary\_word(l)) : binary\_word(l)}: make NOT bitwise operation on a binary word of length \styleIMI{l}
\end{itemize}
...
\styleIMI{property := \#synth CycleThrough(state\_predicate\_1, \dots, state\_predicate\_n);}
...
\styleIMI{property := \#synth CycleThrough(loc[pta1] = l1, loc[pta1] = l2 and loc[pta2] = l2, accepting, i = 2 or i = 3);}

A cycle is accepting if it contains:
\begin{itemize}
	\item a state in which \styleIMI{pta1} is in location \styleIMI{l1}; and
	\item another state in which \styleIMI{pta1} is in location \styleIMI{l1} and at the same time \styleIMI{pta2} is in location \styleIMI{l2}; and
