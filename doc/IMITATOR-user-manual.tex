...
furthermore, \IPTA{} augment the expressiveness of PTA with several features such as invariants, discrete (integer) variables, complex guards and invariants (\ie{} not only comparing a single clock to a single parameter), stopwatches (\ie{} the ability to stop clocks in some locations), and arbitrary clock updates (\ie{} not necessarily to~0).
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Inverse Method}\label{ss:mode:IM}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
The path can be either relative (to the path to the \imitatorExec{} binary) or absolute, and must be followed by the file name.
...
%------------------------------------------------------------
\imitator{} considers that all the \IPTA{} declaring a given synchronization label must be able to synchronize all together, so that the synchronization can happen.
By default, \imitator{} considers that the synchronization labels declared in an \IPTA{} are those declared in the \styleIMI{synclabs} section.
Therefore, if a synchronization label is declared but never used in (at least) one \IPTA{}, this label will never be synchronized in the execution\footnote{In such a case, the synchronization label is actually completely removed before the execution, in order to optimize the execution, and the user is warned of this removal.}.

The option \styleOption{-sync-auto-detect} allows to detect automatically the synchronization labels in each \IPTA{}: the labels declared in the \styleIMI{synclabs} section are ignored, and \imitator{} considers that only the labels really used in an \IPTA{} are those considered to be declared.
...
%------------------------------------------------------------
\begin{tabular}{l l}
	\  & \npec{\styleIMI{print} \styleIMI{(} \styleIMI{reach} \styleIMI{forward} \styleIMI{from} \styleIMI{init} \styleIMI{endreach} \styleIMI{)} \styleIMI{;}} \\
	$|$ & \emptystring \\
\end{tabular}
...
Giuseppe~Lipari provided examples from the real-time systems community.
