...
Note the difference between constraints initializations and discrete variables initializations. The first are initialized in the \styleIMI{continuous} part while other are initialized in the \styleIMI{discrete} part. Moreover, discrete variables are initialized with the assignment operator \styleIMI{:=} in contrast with unification operator \styleIMI{=} used for constraints. As all discrete variables are independent, the initializations are separated with a comma.
...
Init \styleIMI{continuous} part must contain the initial constraints for clocks and parameters.
