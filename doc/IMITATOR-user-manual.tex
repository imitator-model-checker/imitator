...
Int variables are encoded in \imitator{} using the \styleFileContent{int} type of OCaml.
...
\begin{becareful}[int encoding]
	...
	For critical models, it is highly advised to use \emph{rational} variables instead (the computation using rational variables is slower but always exact).
\end{becareful}
...
where \styleIMI{r1}, \styleIMI{r2} are discrete rational variables, \styleIMI{x1}, \styleIMI{x2} are clocks and \styleIMI{p1}, \styleIMI{p2} are parameters.
