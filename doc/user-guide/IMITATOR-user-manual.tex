...
For algorithms using \styleTCTL{AU} (\ie{} \styleTCTL{AF}, \styleTCTL{AU}, \styleTCTL{AR}, \styleTCTL{AW}, \styleTCTL{EG}), together with their timed extension, an option \styleOption{-no-cache-in-AF} might be useful, that disables a caching system consisting in keeping in memory the result of \styleTCTL{AU} on a node, at the cost of more memory.
Disabling it using option \styleOption{-no-cache-in-AF} may result in less memory at the price of a slower analysis.
...
\begin{figure}[tb]
	\centering
	\includegraphics[width=.4\textwidth]{ex-nonparam-2signals-2_ex_1_pos.pdf}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Given a Parametric Timed Game (PTA enriched with controllable/uncontrollable actions) synthesizes a parameter constraint such that, for any parameter valuation in that constraint, a winning strategy exists for the controllable player~\cite{DBLP:conf/tacas/DahlsenJensenFPP24}. Based on the algorithm on Timed Games described in~\cite{DBLP:conf/concur/CassezDFLL05} and further extended for Parametric Timed Games in \cite{DBLP:conf/wodes/JovanovicFLR12}.
...
This synthesizes a parameter constraint guaranteeing a winning strategy for any parameter valuation within it. A winning strategy dictates when to take controllable actions (or wait, forcing the environment to act) such that - regardless of uncontrollable actions - the state predicate is eventually satisfied (e.g., reaching an accepting location).

\subsection{Strategy Generation}\label{section:algorithm:ptg:strategy}
During execution, the algorithm constructs a winning strategy, which is output as a textual representation upon termination. This strategy guarantees that the state predicate is eventually satisfied for all runs starting from parameter valuations within the synthesized constraint. To disable strategy output, use \styleOption{-PTG-no-strategy-printing}. To disable construction of a winning strategy entirely see \cref{section:algorithm:ptg:no-strategy}.

\subsection{Controller Generation}\label{section:algorithm:ptg:controller}  
After the main algorithm terminates (and strategy output completes, unless disabled), the controller generation procedure begins. This step converts the winning strategy into a timed automaton controller, saved as an \imitator file. When composed in parallel with the original game, this controller enforces the strategy, guaranteeing that the state predicate is eventually satisfied in all runs.



\subsection{Disabling Strategy Generation}\label{section:algorithm:ptg:no-strategy}
To disable strategy generation entirely (including computation, printing, and controller conversion), use \styleOption{-PTG-no-strategy-generation}. This significantly accelerates the algorithm on larger examples when only the parameter constraint synthesis is needed.
...
\begin{figure}
	...
	\begin{subfigure}[b]{.48\textwidth}
		\includegraphics[width=\textwidth]{fischerPAT_obs-statespace-nodetails.pdf}

		\caption{No details}
		\label{figure:statespace:nodetail}
	\end{subfigure}
	\begin{subfigure}[b]{.48\textwidth}
		\includegraphics[width=\textwidth]{fischerPAT_obs-statespace-normal.pdf}
	...
	\begin{subfigure}[b]{.75\textwidth}
		\includegraphics[width=\textwidth]{fischerPAT_obs-statespace-verbose.pdf}
...
%------------------------------------------------------------
\paragraph{\styleOption{-no-cache-in-AF} (default: disabled, \ie{} cache is used)}
%------------------------------------------------------------
For algorithms using \styleTCTL{AU} (\ie{} \styleTCTL{AF}, \styleTCTL{AU}, \styleTCTL{AR}, \styleTCTL{AW}, \styleTCTL{EG}), together with their timed extension, this option disables a caching system consisting in keeping in memory the result of \styleTCTL{AU} on a node, at the cost of more memory.
Disabling the cache using this option may result in less memory at the price of a slower analysis.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Contact}\label{chapter:contact}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

To get in touch in case of any question or issue (install, help to write models, bug spotting…), the easiest is to get in touch via a GitHub issue:

\url{https://github.com/imitator-model-checker/imitator/issues}
...
\begin{center}
	\includegraphics[width=.3\textwidth]{GPLv3_Logo.png}
\end{center}
...
\begin{center}
	\includegraphics[width=.2\textwidth]{CC-BY-SA_500.png}
\end{center}
