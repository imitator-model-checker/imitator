...
% \begin{figure}
    ...
    \begin{scope}[xshift=15cm]
      ...
      \path (q0) edge[left,->, line width=1.5] node {\actlab{safe}} (q1);
      \path (q0) edge[right,->, line width=1.5] node {\actlab{crash}} (q2);
      \path (q0) edge[loop above, line width=1.5] node {\actlab{bounce}} (q0);
      \path (INIT) edge[->,bend right=30, line width=1.5] (q0);      
    \end{scope}
...
\begin{itemize}
	\item \coulitem{Hybrid Automata} (HA): Sets of \vari{variables}, \actlab{actions}, locations, % (with an activity and an invariant)
		and discrete transitions % (with jumps).
	\item \coulitem{Parameterized Hybrid Automata}: HA augmented with a set of \param{parameters} (unknown constants)
	\vspace{1cm}
...

{
  ...
  \small
  \begin{tikzpicture}[scale=\scalepump * 0.4,very thick,->,>=stealth']
    ...
    (ON)  edge[bend left=30] node[above left] % , anchor=right,left
    {
      \begin{tabular}{r}
        $\vari{w} = \param{M} /$ \\
    ...
    (START)  edge[bend left=30] node[below left]
    {
      \begin{tabular}{r}
        $\vari{t} = \param{delay} /$ \\
        \actlab{on}
    ...
    (STOP)  edge[bend left=30] node[above right]
    {
      \begin{tabular}{l}
        $\vari{w} > \param{max} /$ \\
        \actlab{overflow}
    ...
    (START)  edge[bend right=30] node[below right]
    {
      \begin{tabular}{l}
        $\vari{w} < \param{min} /$ \\
        \actlab{underflow}
    ...
    \node at (-5,-8.5) [draw=none] {\refer{HPR97}}; % 
...
\begin{block}{The Parameter Synthesis Problem}
  ...
  \begin{tikzpicture}[scale=\scalepump * 0.35]
    ...
    \draw[->, line width=1.2] (0,0) -- (0,12); \draw (0,13) node {$w$};
    \draw[->, line width=1.2] (0,0) -- (16,0); \draw (17.4,0) node [below] {$time$};
...
Synthesis problem: ``\couleur{find values for the parameters such that the system behaves well}''.

\vspace{1cm}

We will synthesize here a \couleur{constraint}, viz., a convex and dense set of values.
...
\begin{itemize}
  ...
  \begin{tikzpicture}[scale=1]
    ...
    \begin{scope}[xshift=0cm]
	\tikzstyle{etat}=[location, inner sep=5pt]
      \draw (0,0) node[etat] (Q0) {$s_0$};
      \draw (3,0) node[etat] (Q1) {$s_1$};
      \draw (6,2) node[etat] (Q2) {$s_2$};
      \draw (6,-2) node[etat] (Q3) {$s_3$};
      \draw (9,-2) node[etat] (Q4) {$s_4$};
      \draw (12,1) node[etat] (Q5) {$s_5$};
      \draw (12,-2) node[etat] (Q6) {$s_6$};
      ...
      \draw (6,2) node[etat,color=gray!30,fill=gray!10,text=gray!30] (Q2b) {$s_2$};
      \path (Q1) edge[->,double,color=gray!30] (Q2);      
      \draw[color=red,very thick] (5,3) -- (7,1);
      \draw[color=red,very thick] (5,1) -- (7,3);

      \draw (12,1) node[etat,color=gray!30,fill=gray!10,text=gray!30] (Q5b) {$s_5$};
