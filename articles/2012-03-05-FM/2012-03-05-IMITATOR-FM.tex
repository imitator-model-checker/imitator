...
There are many possible schedules (two of them are depicted in Fig.~\ref{fig:schedules_maler} in Appendix). 
%
...
(For a geometrical representation of $\Ko$, see Fig.~\ref{fig:geom} of Appendix.)
...
(This set of traces is depicted under the form of a tree in Fig.~\ref{fig:output} in Appendix.)
...
Another example of application of \imitator{} 2.5 to a scheduling problem, originating from \cite{bb04}, is given in Appendix \ref{app:carto} (using 
the cartography method \cite{af10}). 
...
However, in many scheduling problems,
...
\section*{Appendix: Case Study from \cite{AM02}}
...
\section*{Appendix: Case Study from \cite{bb04}}
\label{app:carto}
We present here what we can obtain from \imitator{} 2.5 with the Behavioral Cartography (BC) output. By iterating $\IM$ over all the integers points inside a finite rectangle $V_0$, one is able to decompose
(most of) the parametric space included into $V_0$ into behavioral tiles\footnote{Actually, one can take a finer discretization step to ensure a better coverability of $V_0$}.
...
have to test the schedulability of a single point for each tile to test wether or not $\A$ is schedulable for any point in this tile. We apply this method on the example presented in \cite{bb04}.

We have three periodic tasks $\tau_1, \tau_2$ and $\tau_3$ with periods of $3,8$ and $20$ respectively and deadlines of $3,8$ and $20$ respectively. We are interested in finding the set of computation times
 of each task such that the system is schedulable. The interested reader can find the full details in \cite{bb04}.

We set $V_0$ as $C_1 \in [0,3]$, $C_2 \in [0,8]$ and $C_3 \in [0,20]$ because the system is unschedulable if there exists a task $\tau_i$ such that $C_i > T_i$. 
...
The result for this example is given in Figure \ref{fig:carto}, for $C_1,C_2$ left as parameters and $C_3$ set to $0$, for the sake of lisibility but the method works for an arbitrary number of parameters.

\begin{figure}[!ht]
 \centering
 \includegraphics[scale = 0.5]{./figures/C1-C2.png}
 \caption{Schedulability, in green the system is schedulable, in red, unschedulable}
 \label{fig:carto}
\end{figure}
It is interesting to notice that, we covered the full parameter space and, therefore solved the parametric schedulability problem by simply checking the schedulability of 20 valuations of the parameters
, as there is only 20 tiles.
