...
Different from CEGAR-based methods (see \cite{cgjlv00}), this algorithm for parameter synthesis
makes use of a ``good'' parameter valuation~$\pio$ instead of a set of ``bad'' states~\cite{acef09}.
...
Actually, most of the scheduling examples we consider run out of memory without the merging technique.
...
There are many possible schedules; two of them are depicted in Fig.
\ref{fig:schedules_maler} (see Appendix). 
%
...
In  \cite{AM02}, this problem is modeled as a product ${\cal A}$ of TAs
with stopwatches, each TA corresponding to a job.
Each schedule corresponds to a branch in the
reachability tree of ${\cal A}$.
The makespan value corresponds to the
duration of the shortest branch, and is here~9.
...
Let us explain how to analyse the robustness of the
...
synchronized with $\A$, which fires a transition labeled $\mathit{DEADLINE}$
as soon as a schedule spends more than 9 units of time.
...
This yields the constraint $\Ko$:  $7 > d'_2 \wedge 3 > d_2 \wedge d'_2 + d_2 \geq 7$. (For a geometrical representation of $\Ko$,  see Fig.\ref{fig:geom} pf Appendix).
...
This set of traces is depicted under the form of a tree in
Fig. 	\ref{fig:output} (see Appendix).
...
on several classes of scheduling problems (see \cite{soulat12} for details).
...
%parametric model checking for (bounded) Time Petri Nets 
model checking for parametric time Petri nets
with stopwatches, and synthesizes parameter valuations satisfying a TCTL formula.
