...
\author{\'Etienne Andr\'e$^{1,2}$, Laurent Fribourg$^3$, Ulrich K\"uhne$^4$ and Romain Soulat$^3$}
...
any \textit{DEADLINE} label. % (these branches end at node $s73$ in Figure~\ref{fig:output}).
...
% Another example of application of \imitator{} 2.5 to a scheduling problem, originating from \cite{bb04}, is given in Appendix 2 (using the cartography method of~\cite{af10}). 
...
are described in a research report~\cite{soulat12}, and available in~\cite{imitator-web}. %See also \url{http://www.lsv.ens-cachan.fr/Software/imitator/}.
